# anUNet_v3.1: WT-priority training strategy.
...
try:
    from .anUNet_v3_1_model import UNet3D
except ImportError:
    from anUNet_v3_1_model import UNet3D
...
class WTLoss(nn.Module):
    def __init__(self):
        ...
        self.focal = FocalLoss(gamma=2.0, to_onehot_y=False)

    def forward(self, logits: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        # Convert multi-class labels to binary WT mask for single-head training.
        y_wt = (y > 0).unsqueeze(1).float()
        return self.dice(logits, y_wt) + self.focal(logits, y_wt)
...
def build_train_transforms(patch_size):
    # Data augmentation for robust WT-focused training.
    keys = ["image", "label"]
    return Compose([
...
def build_val_transforms(patch_size):
    # Deterministic center crop for stable validation.
    return Compose([
...
class NOSA31Lightning(pl.LightningModule):
    def __init__(self):
        super().__init__()
        # Single-output head: predicts WT logits only.
    ...
    def on_validation_epoch_start(self):
        # Reset running confusion-count accumulators for each threshold.
        self._val_stats = {
            thr: {
    ...
    def validation_step(self, batch, batch_idx):
        # Validation pass: evaluate WT predictions across threshold sweep.
        x, y = batch
        logits = self(x)
        loss = self.loss_fn(logits, y)
    ...
    def on_validation_epoch_end(self):
        # Select best WT threshold based on IoU.
...
def build_first_fold_case_split(
    ...
    # Hybrid split: GroupKFold for mapped cases + KFold fallback for unmapped cases.
...
def main():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA GPU is required for NOSA_v3.1 WT-priority training.")

    enable_tf32()

    os.environ["WANDB_MODE"] = "online"

    # Build training/validation case IDs for selected fold.
    ...
    # Build datasets and apply case-level split.
    ...
    wandb_logger = WandbLogger(project="NOSA_v3.1", log_model=True, save_dir=MODEL_SAVE_DIR)
    best_thr_path = os.path.join(MODEL_SAVE_DIR, "anUNet_v3_1_best_threshold.json")
    ...
    ckpt_callback = ModelCheckpoint(
        ...
        filename=f"nosa-v3_1-fold{FOLD_INDEX}" + "-{epoch:02d}-{val_wt_iou:.4f}",
