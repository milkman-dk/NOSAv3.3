# anUNet_v3.2: WT-priority training strategy.
...
try:
    from .anUNet_v3_2_model import UNet3D
except ImportError:
    from anUNet_v3_2.anUNet_v3_2_model import UNet3D
...
class WTLoss(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        # Convert labels to binary WT target for single-head objective.
...
def build_train_transforms(patch_size):
    # Randomized geometric + intensity augmentation for train-time robustness.
    keys = ["image", "label"]
    return Compose([
...
def build_val_transforms(patch_size):
    # Deterministic center crop for evaluation consistency.
    return Compose([
...
class NOSA31Lightning(pl.LightningModule):
    def __init__(self):
        super().__init__()
        # Single-channel output for WT probability logits.
    ...
    def on_validation_epoch_start(self):
        # Reset per-threshold TP/FP/FN counters each epoch.
        self._val_stats = {
            thr: {
    ...
    def validation_step(self, batch, batch_idx):
        # Validation pass with threshold sweep bookkeeping.
        x, y = batch
        logits = self(x)
        loss = self.loss_fn(logits, y)
    ...
    def on_validation_epoch_end(self):
        # Pick threshold with highest WT IoU.
...
def build_first_fold_case_split(
    ...
    # Hybrid fold split using GroupKFold plus fallback KFold.
...
def main():
    # Setup and deterministic initialization.
    set_seed(SEED)

    if not torch.cuda.is_available():
        raise RuntimeError("CUDA GPU is required for anUNet_v3.2 WT-priority training.")

    enable_tf32()

    os.environ["WANDB_MODE"] = "online"

    # Compute fold-specific case split.
    ...
    # Construct datasets and apply selected case IDs.
    ...
    wandb_logger = WandbLogger(project="anUNet_v3.2", log_model=True, save_dir=MODEL_SAVE_DIR)
    best_thr_path = os.path.join(MODEL_SAVE_DIR, "anUNet_v3_2_best_threshold.json")
    ...
    ckpt_callback = ModelCheckpoint(
        ...
        filename=f"nosa-v3_2-fold{FOLD_INDEX}" + "-{epoch:02d}-{val_wt_iou:.4f}",
