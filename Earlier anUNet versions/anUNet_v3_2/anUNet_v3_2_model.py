...
class UNet3D(nn.Module):
    # 3D residual U-Net with anisotropic early downsampling for BraTS volumes.
    ...
    def __init__(
        ...
        self.enc2 = ResidualStage3D(c1, c2, num_blocks=stage_blocks, dropout=0.05)
        self.down2 = Downsample3D(c2, stride=(2, 2, 2))

        self.enc3 = ResidualStage3D(c2, c3, num_blocks=stage_blocks, dropout=0.10)
        self.down3 = Downsample3D(c3, stride=(2, 2, 2))

        self.enc4 = ResidualStage3D(c3, c4, num_blocks=stage_blocks, dropout=0.10)
        ...
        self.up1 = UpBlock3D(cb, c4, d1, stride=(2, 2, 2), dropout=0.10, num_blocks=stage_blocks)
        self.up2 = UpBlock3D(d1, c3, d2, stride=(2, 2, 2), dropout=0.10, num_blocks=stage_blocks)
        self.up3 = UpBlock3D(d2, c2, d3, stride=(2, 2, 2), dropout=0.05, num_blocks=stage_blocks)
        self.up4 = UpBlock3D(d3, c1, d4, stride=(1, 2, 2), dropout=0.05, num_blocks=stage_blocks)
